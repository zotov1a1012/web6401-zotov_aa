...
// Функция для отправки данных формы через POST-запрос на локальный сервер
async function submitFormData(userData) {
    try {
        console.log('📤 Отправка данных на сервер:', userData);
        
        // Пробуем отправить на локальный mock-json-server
        let response;
        try {
            response = await fetch('http://localhost:3000/users', {
                ...
                body: JSON.stringify({
                    // Отправляем данные пользователя в структуре, соответствующей db.json
                    ...userData,
                    createdAt: new Date().toISOString(),
                    id: Date.now() // временный ID, сервер присвоит свой
                }),
            });
        } catch (e) {
            console.log('❌ Локальный сервер недоступен:', e.message);
        ...
        return {
            id: Date.now(),
            ...userData,
            message: 'Сервер вернул ошибку, данные сохранены локально',
            simulated: true,
            error: error.message
        };
    }
